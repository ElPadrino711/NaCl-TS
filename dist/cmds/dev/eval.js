...
exports.run = async (d) => {
    ...
    if (!config.devs.includes(author.id))
        return msg.reply(':x:  |  No puedes usar esto!').then((x) => setTimeout(() => x.delete(), 3000));
    if (!args[0])
        return d.msg.reply(':x:  |  Escribe algo que evaluar!').then((x) => setTimeout(() => x.delete(), 3000));
    var evaluado, tipo;
    try {
        ...
        if (evaluado.length >= 1024) {
            var sb = create([{ content: `${evaluado.replace(bot.token, '[ToKeN]')}`, language: 'javascript' },], { title: 'Eval', description: 'result >= 1024' });
...
exports.help = {
    ...
    inDev: !0,
    usage: async (d) => { return d.prefixes[0] + 'eval <Codigo para evaluar>'; }
};
